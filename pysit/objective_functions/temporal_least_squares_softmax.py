...
__all__ = ['TemporalLeastSquaresSoftmax']

__docformat__ = "restructuredtext en"

class TemporalLeastSquaresSoftmax(ObjectiveFunctionBase):
    ...
    def _softmax(self, inputs):
        ...
        #softmax3 = softmax1 - epsl*np.log(1 + np.exp(-inputs/epsl))
        #derivative_softmax3 = derivative_softmax1 + (np.exp(-inputs/epsl)/(1 + np.exp(-inputs/epsl)))
        #softmax4 = -epsl*np.log(1 + np.exp(-inputs/epsl))
        #derivative_softmax4 = np.exp(-inputs/epsl) / (1 + np.exp(-inputs/epsl))   
    ...
    def _residual(self, shot, m0, dWaveOp=None, wavefield=None):
        ...
        return resid, adjoint_source_l2_softmax
